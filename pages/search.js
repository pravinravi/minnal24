...
export default function SearchPage() {
  ...
  return (
    <Layout>
      <NextSeo title={q ? `"${q}" தேடல்` : 'தேடல்'} noindex />
      <div className="bg-red-700 py-8">
        <div className="max-w-7xl mx-auto px-4">
          <h1 className="text-2xl font-black text-white tamil-text mb-4">
            {q ? `"${q}" தேடல் முடிவுகள்` : 'தேடுங்கள்'}
          </h1>
          <form action="/search" method="get" className="flex gap-2">
            <input type="search" name="q" defaultValue={q}
              placeholder="செய்திகளை தேடுங்கள்..."
              className="flex-1 px-5 py-3 rounded-xl text-gray-900 focus:outline-none tamil-text text-base" />
            <button type="submit"
              className="bg-yellow-400 text-gray-900 px-6 py-3 rounded-xl font-black tamil-text">
              தேடு
            </button>
          </form>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            {loading ? (
              <p className="text-center py-10 tamil-text text-gray-500">தேடுகிறோம்...</p>
            ) : q && results.length === 0 ? (
              <div className="text-center py-16">
                <div className="text-6xl mb-4">🔍</div>
                <p className="text-gray-500 tamil-text">"{q}" க்கு முடிவு இல்லை</p>
              </div>
            ) : (
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map(cat => (
                  <a key={cat.id} href={`/category/${cat.slug}`}
                    className="px-3 py-1.5 rounded-full text-sm font-semibold tamil-text"
