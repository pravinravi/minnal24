...
export default function Header() {
  ...
  useEffect(() => {
    const updateTime = () => {
      const now = new Date();
      setCurrentTime(now.toLocaleString('ta-LK', {
  ...
  const mainNavItems = categories.slice(1, 8);
  ...
  return (
    <>
      ...
      {/* Main header */}
      <header className={`bg-white sticky top-0 z-50 transition-shadow duration-300 ${scrolled ? 'shadow-lg border-b-2 border-red-600' : 'shadow-sm'}`}>
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between py-2 md:py-3">
            ...
            <Link href="/" className="flex-shrink-0">
              <div className="flex items-center">
                <div className="relative h-12 w-48 md:h-14 md:w-56">
                  {/* Logo fallback with styled text if image not loaded */}
                  <div className="flex items-center gap-1 h-full">
                    <div className="text-red-700 font-black text-2xl md:text-3xl tamil-text leading-none tracking-tight">
                      மின்னல்
                    </div>
                    <div className="text-yellow-500 font-black text-3xl md:text-4xl leading-none" style={{ fontFamily: 'Bebas Neue, Impact, sans-serif' }}>
                      24
                    </div>
                    <div className="text-red-600 text-xs font-bold leading-none">.com</div>
                  </div>
                </div>
              </div>
            </Link>

            {/* Desktop ad banner space in header */}
            <div className="hidden lg:flex flex-1 max-w-md mx-6">
              <div className="w-full h-16 rounded-lg border-2 border-dashed border-gray-200 bg-gray-50 flex items-center justify-center">
                ...
                <span className="text-xs text-gray-300">Leaderboard Ad</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <button
                ...
                className="md:hidden p-2 rounded-full text-gray-600 hover:bg-red-50 hover:text-red-600 transition-colors"
                aria-label="Menu"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  {mobileOpen
          ...
          {/* Search bar */}
          {searchOpen && (
            <div className="pb-3 border-t border-gray-100 pt-2 animate-fade-in">
              <form action="/search" method="get" className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 border-2 border-gray-200 rounded-lg focus:border-red-500 focus:outline-none tamil-text text-sm"
                  autoFocus
                />
                <button type="submit"
                  className="bg-red-600 text-white px-5 py-2.5 rounded-lg hover:bg-red-700 transition-colors font-medium text-sm tamil-text">
                  தேடு
                </button>
              </form>
            </div>
        ...
        <nav className="bg-red-700 hidden md:block">
          <div className="max-w-7xl mx-auto px-4">
            <ul className="flex items-center overflow-x-auto scrollbar-hide">
              <li>
                <Link href="/"
                  className="flex items-center gap-1.5 px-4 py-3 text-white text-sm font-bold hover:bg-red-800 transition-colors whitespace-nowrap tamil-text">
        ...
        {/* Mobile menu */}
        <div className={`mobile-menu md:hidden bg-gray-900 ${mobileOpen ? 'open' : ''}`}>
